...
public class ReplyService {
        ...
        @Transactional
        public ReplyResponse createReply(Long workplaceId, Long reviewId, ReplyCreateRequest req, Long employerUserId,
                        boolean isAdmin) {
                requireWorkplace(workplaceId);
                Review review = requireReview(workplaceId, reviewId);
        ...
        @Transactional
        public ReplyResponse updateReply(Long workplaceId, Long reviewId, ReplyUpdateRequest req, Long employerUserId,
                        boolean isAdmin) {
                requireWorkplace(workplaceId);
        ...
        public void deleteReply(Long workplaceId, Long reviewId, Long actingUserId, boolean isAdmin) {
                ...
                // Skip employer check if admin (actingUserId may be null for admin operations)
                if (!isAdmin) {
                        if (actingUserId == null) {
                                throw new HandleException(ErrorCode.ACCESS_DENIED, "Authentication required");
                        }
                        assertEmployerOrAdmin(workplaceId, actingUserId, false);
                }
