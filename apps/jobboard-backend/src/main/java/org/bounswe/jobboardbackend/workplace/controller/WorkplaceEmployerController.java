...
public class WorkplaceEmployerController {
        ...
        public ResponseEntity<List<EmployerWorkplaceBrief>> myWorkplaces(Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal();
                return ResponseEntity.ok(employerService.listWorkplacesOfEmployer(userDetails.getId()));
        }
        ...
        public ResponseEntity<PaginatedResponse<EmployerRequestResponse>> listRequests(
                        ...
                        Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal();
                var res = employerService.listRequests(workplaceId, page, size, userDetails.getId(), isAdmin(auth));
        ...
        public ResponseEntity<ApiMessage> createRequest(
                        ...
                        Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal(); // başvuru yapan
                employerService.createRequest(workplaceId, req, userDetails.getId());
        ...
        public ResponseEntity<EmployerRequestResponse> getRequest(
                        ...
                        Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal();
                var res = employerService.getRequest(workplaceId, requestId, userDetails.getId(), isAdmin(auth));
                return ResponseEntity.ok(res);
        }
        ...
        public ResponseEntity<ApiMessage> resolve(
                        ...
                        Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal();
                employerService.resolveRequest(workplaceId, requestId, req, userDetails.getId(), isAdmin(auth));
                return ResponseEntity.ok(
                                ApiMessage.builder().message("Employer request resolved")
                                                .code("EMPLOYER_REQUEST_RESOLVED").build());
        }
        ...
        public ResponseEntity<ApiMessage> removeEmployer(
                        ...
                        Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal();
                employerService.removeEmployer(workplaceId, employerId, userDetails.getId(), isAdmin(auth));
                return ResponseEntity
                                .ok(ApiMessage.builder().message("Employer removed").code("EMPLOYER_REMOVED").build());
        }
        ...
        public ResponseEntity<PaginatedResponse<EmployerRequestResponse>> myRequests(
                        ...
                        Authentication auth) {
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal();
                var res = employerService.listMyRequests(userDetails.getId(), page, size);
                return ResponseEntity.ok(res);
        }
}
