...
const mockWorkplaceDetail: WorkplaceDetailResponse = {
  ...
  shortDescription: 'Leading tech innovation',
  detailedDescription:
    'We are a global leader in technology innovation, committed to creating a better future through software.',
...
export const workplaceHandlers = [
  ...
  http.post(`${API_BASE_URL}/workplace/:workplaceId/review/:reviewId/helpful`, async () => {
    // Simulate successful helpful vote
    return HttpResponse.json(
      {
        success: true,
        helpfulCount: Math.floor(Math.random() * 10) + 1, // Random count for demo
        userVoted: true,
  ...
  http.delete(`${API_BASE_URL}/workplace/:workplaceId/review/:reviewId/helpful`, async () => {
    // Simulate successful helpful vote removal
    return HttpResponse.json(
      {
        success: true,
        helpfulCount: Math.floor(Math.random() * 10), // Random count for demo
        userVoted: false,
  ...
  http.get(`${API_BASE_URL}/workplace/:workplaceId/reviews/helpful-status`, async ({ request }) => {
    const url = new URL(request.url);
    const reviewIds =
      url.searchParams
        .get('reviewIds')
    ...
    // Simulate user vote status - randomly vote on some reviews
    const voteStatus: Record<number, boolean> = {};
    reviewIds.forEach((reviewId) => {
      voteStatus[reviewId] = Math.random() > 0.5; // Randomly voted
    });

    return HttpResponse.json(voteStatus, { status: 200 });
  }),
];
