...
export const updateWorkplaceSchema = z.object({
  companyName: z
    .union([z.string().min(1, 'Company name is required').max(255), z.literal('')])
    .optional(),
  sector: z
    .union([z.string().min(1, 'Sector is required').max(100), z.literal('')])
    .optional(),
  location: z
    .union([z.string().min(1, 'Location is required').max(255), z.literal('')])
    .optional(),
