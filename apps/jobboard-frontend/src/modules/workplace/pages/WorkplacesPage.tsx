...
import EmployerWorkplacesPage from './EmployerWorkplacesPage';
...
export default function WorkplacesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'browse' | 'my'>(
    isEmployer && tabParam === 'my' ? 'my' : 'browse'
  );
  ...
  useEffect(() => {
    if (!isEmployer && activeTab === 'my') {
      setActiveTab('browse');
      return;
    }
    if (isEmployer && tabParam === 'my' && activeTab !== 'my') {
      setActiveTab('my');
    }
  }, [isEmployer, tabParam, activeTab]);
  ...
  const handleTabChange = (value: string) => {
    const nextTab: 'browse' | 'my' = value === 'my' && isEmployer ? 'my' : 'browse';
    ...
    <div className="bg-background min-h-screen">
      <div className="container mx-auto px-4 py-6">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          <TabsList className={isEmployer ? 'grid w-full max-w-xl grid-cols-2' : 'grid w-full max-w-xs grid-cols-1'}>
            <TabsTrigger value="browse">{t('workplaces.tabs.browse', 'Browse')}</TabsTrigger>
            {isEmployer && (
              <TabsTrigger value="my">{t('workplaces.tabs.my', 'My Workplaces')}</TabsTrigger>
            )}
          </TabsList>
